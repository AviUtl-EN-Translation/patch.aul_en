...
namespace patch {
    ...
    inline class SharedCache_t {
        ...
        struct SharedCacheInfo { // size = 272
            char name[260];
            int w;
            int h;
            int bitcount;
        ...
        inline static void InitSharedCacheInfo() {
            ...
            if (shared_mem_info_info != NULL) {
                a_exfunc->delete_shared_mem((int)&shared_mem_info_info, shared_mem_info_info);
            }
            for (int i = 0; i < cache_count; i++) {
                if (shared_mem_info[i] != NULL) {
                    a_exfunc->delete_shared_mem((int)&shared_mem_info[i], shared_mem_info[i]);
                }
            }
            cache_count = 0;
            priority_count = 1;
        ...
        inline static SharedCacheInfo* GetOrCreateSharedCacheInfo() {
            ...
            if (auto sci = static_cast<SharedCacheInfo*>(a_exfunc->get_shared_mem(std::bit_cast<int32_t>(&shared_mem_info_info), 0, shared_mem_info_info)); sci != nullptr) return sci;
