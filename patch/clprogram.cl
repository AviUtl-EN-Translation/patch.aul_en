...
kernel void blend_yc_yc_normal(global short* dst, global short* src, int dst_line, int src_line, int alpha) {
	int x = get_global_id(0);
	int y = get_global_id(1);

	dst = (global short*)((int)dst + x * 6 + y * dst_line);
	src = (global short*)((int)src + x * 6 + y * src_line);

	dst[0] += (short)((src[0] - dst[0]) * alpha >> 12);
	dst[1] += (short)((src[1] - dst[1]) * alpha >> 12);
	dst[2] += (short)((src[2] - dst[2]) * alpha >> 12);
}
kernel void blend_yc_yca_normal(global short* dst, global short* src, int dst_line, int src_line, int alpha) {
	int x = get_global_id(0);
	int y = get_global_id(1);

	dst = (global short*)((int)dst + x * 6 + y * dst_line);
	src = (global short*)((int)src + x * 8 + y * src_line);

	int src_a = src[3] * alpha >> 12;

	dst[0] += (short)((src[0] - dst[0]) * src_a >> 12);
	dst[1] += (short)((src[1] - dst[1]) * src_a >> 12);
	dst[2] += (short)((src[2] - dst[2]) * src_a >> 12);
}
kernel void blend_yca_yc_normal(global short* dst, global short* src, int dst_line, int src_line, int alpha) {
	int x = get_global_id(0);
	int y = get_global_id(1);

	dst = (global short*)((int)dst + x * 8 + y * dst_line);
	src = (global short*)((int)src + x * 6 + y * src_line);

	int dst_a = clamp((int)dst[3], 0, 0x1000);
	int new_a = 0x1000800 - (0x1000 - dst_a) * (0x1000 - alpha) >> 12;
	dst_a = (0x1000 - alpha) * dst_a / new_a;
	int src_a = (alpha << 12) / new_a;

	dst[0] = (short)(dst[0] * dst_a + src[0] * src_a >> 12);
	dst[1] = (short)(dst[1] * dst_a + src[1] * src_a >> 12);
	dst[2] = (short)(dst[2] * dst_a + src[2] * src_a >> 12);
	dst[3] = (short)new_a;
}
kernel void blend_yca_yca_normal(global short* dst, global short* src, int dst_line, int src_line, int alpha) {
	int x = get_global_id(0);
	int y = get_global_id(1);
	...
	if (0 < src_a) {
		int dst_a = clamp((int)dst[3], 0, 0x1000);
		int new_a = 0x1000800 - (0x1000 - dst_a) * (0x1000 - src_a) >> 12;
		dst_a = (0x1000 - src_a) * dst_a / new_a;
		src_a = (src_a << 12) / new_a;
